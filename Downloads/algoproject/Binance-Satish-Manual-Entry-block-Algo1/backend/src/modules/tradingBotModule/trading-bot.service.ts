...
//console.log(`Checking patterns for candle, Symbol: ${symbol}, Interval: ${interval}`,candle1,candle2,candle3);
      if (this.isHammer(candle1)) {
        ...
        if (candle2.low !== candle2.open && candle2.close > candle1.high && candle3.high > candle2.high && candle3.low > candle2.low && candle2.low > candle1.low) {
          try {
            ...
            const baseWindows = [1440, 720, 240, 180, 120, 60,15];
    ...
    try {
      ...
      for (let i = 4; i < candles.length; i++) {
        ...
        if (patternFound) {
          ...
          if (wantBullish && currentPrice <= c2.low) continue;
          if (!wantBullish && currentPrice >= c2.high) continue;
          if (wantBullish && currentPrice >= c4.high) continue;
          if (!wantBullish && currentPrice <= c4.low) continue;
          return true;
        }
        for (let numInside = 5; numInside >= 1; numInside--) {
          ...
          if (patternFound) {
            ...
            const currentPrice = this.candles1m[symbol]?.[this.candles1m[symbol].length - 1]?.close || 0;
            if (wantBullish && currentPrice <= breakout.low) continue;
            if (!wantBullish && currentPrice >= breakout.high) continue;
            if (wantBullish && currentPrice >= confirm.high) continue;
            if (!wantBullish && currentPrice <= confirm.low) continue;
            return true;
          }
        }
      }
  ...
  private getUserConfig(userId: string): UserTradingConfig {
    const configs: Record<string, UserTradingConfig> = {
      '68d3f007099f9de77119b413': {
        API_KEY: 'VJRBHzUzY7efoCpZtrDMQLRVDHJUzLy6wmfy1bN13MASkXuMiM7VaMsLkA6ekIbB',
        API_SECRET: 'k5Tmtkt5O7Kjd6soAFhewPFaCjC2Jx0JawjuXaxjzXL3y5zC5l7q4ifWwTRvLDrC',
        RISK_PERCENT: 0.4,
        LEVERAGE: 15,
        MARGIN_TYPE: 'ISOLATED',
        HEDGE_MODE: true,
      },
      'sub1-68d3f007099f9de77119b413': { API_KEY: 'xhvQqDNyh7StyCrKcArH8cCkdyycJKcyZBHZjndkWXjs8RqY9KSbqIK9hH0Yt7rI', API_SECRET: 'aZSNaQaI4b4Qz021jQnY1VepO7aRIkHDljX2WdcdvC0Ih2FonURmJ9F0sbLe2von', RISK_PERCENT: 0, LEVERAGE: 0, MARGIN_TYPE: 'ISOLATED', HEDGE_MODE: true },
