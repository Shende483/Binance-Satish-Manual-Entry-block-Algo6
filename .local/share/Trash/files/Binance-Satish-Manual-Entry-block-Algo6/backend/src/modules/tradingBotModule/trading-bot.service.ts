    ...
    try {
      ...
      for (let i = 4; i < candles.length; i++) {
        ...
        if (wantBullish && this.isHammer(c1) &&
          ...
          currentPrice > c2.low && currentPrice < c2.high
          ) {
          patternFound = true;
          patternLow = c2.low;
        ...
        if (!wantBullish && this.isInverseHammer(c1) &&
          ...
          currentPrice < c2.high && currentPrice > c2.low
          ) {
          patternFound = true;
          patternHigh = c2.high;
        ...
        if (wantBullish &&
            ...
            c3.open !== c3.low &&
            currentPrice > c3.low && currentPrice < c3.high
          ) {
          patternFound = true;
          patternLow = c3.low;
          firstCandleLow = c1.low;
        ...
        if (!wantBullish &&
            ...
            c3.open !== c3.high &&
            currentPrice < c3.high && currentPrice > c3.low
          ) {
          patternFound = true;
          patternHigh = c3.high;
          firstCandleHigh = c1.high;
 ...
 // console.log(`All checks passed → returning true for `);
  return true;
        ...
        for (let numInside = 5; numInside >= 1; numInside--) {
          ...
          confirm.high > breakout.high &&
          confirm.low > breakout.low &&
         currentPrice > breakout.low && currentPrice < breakout.high
            ) {
            patternFound = true;
        ...
        confirm.low < breakout.low &&
        confirm.high < breakout.high &&
         currentPrice < breakout.high && currentPrice > breakout.low) {
            patternFound = true;
...
//  console.log(`Pattern  detected on ${symbol} at ${new Date(mother.startTime).toISOString()} in ${interval}`);
 ...
 // console.log(`Inside Bar History candles count: ${historyCandles.length}, from ${historyStart} to ${motherIndex-1}`);
  ...
  private isTradingHours(): boolean {
   ...
   // const startTradingUTC = 4 * 60 + 30;
     const startTradingUTC = 14 * 60 + 30;
 ...
 //const balance = 100
    ...
    const riskAmount = ((riskPercent??1.5)/ 100) * balance;
    const rawQty = riskAmount / priceDiff;
